...
- **THEN** valid numeric values for temperature and humidity, along with the current UTC timestamp, SHALL be cached for display and HTTP/CoAP serving
...
- **THEN** the system SHALL read the DHT22 synchronously, update cached sensor values and timestamp, and include the fresh values in the CoAP response
...
- **THEN** the system SHALL respond with the most recently cached values and timestamp without triggering an additional DHT22 read
