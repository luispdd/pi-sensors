...
#### Scenario: Short press from ACTIVE — flush succeeds
- **WHEN** the logger button is short-pressed while in `LOGGER_ACTIVE` and the SD card write succeeds
- **THEN** the system SHALL write all buffered rows, clear the buffer, set `AppState.logging_active` to `False`, set `AppState.logger_state` to `LOGGER_IDLE`, and briefly display a success message (e.g. `"Flushed!"`)

#### Scenario: Short press from ACTIVE — flush fails
- **WHEN** the logger button is short-pressed while in `LOGGER_ACTIVE` and the SD card write fails
- **THEN** the system SHALL set `AppState.log_error`, display the error, and remain in `LOGGER_ACTIVE` with the buffer retained
