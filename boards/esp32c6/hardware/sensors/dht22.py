"""Modular DHT22 Sensor Driver conforming to the duck-typed sensor protocol, mirroring pico-1w."""
...
class DHT22Sensor:
    ...
    def __init__(self, pin=config.PIN_DHT22, interval_ms=2500):
        self.pin_num = pin
    ...
    def read(self):
        ...
        try:
            ...
            # Single retry after 100ms pause to recover from momentary WiFi interrupt collisions
            try:
                time.sleep_ms(100)
                self._dht_sensor.measure()
                self.last_temp = round(float(self._dht_sensor.temperature()), 1)
        ...
        # Retain and return last known readings if available (matching pico-1w)
...
def create_sensor(pin=config.PIN_DHT22, interval_ms=2500):
